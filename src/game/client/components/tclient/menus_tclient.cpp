...
void CMenus::RenderSettingsTClient(CUIRect MainView)
{
	...
	int TabCount = NUMBER_OF_TCLIENT_TABS - 1;
	...
	if(s_CurCustomTab == TCLIENT_TAB_GORESBOT)
		RenderSettingsGoresBot(MainView);
}
...
void CMenus::RenderSettingsTClientSettings(CUIRect MainView)
{
	...
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_TcKeystrokes, TCLocalize("Show keystroke HUD (RMB A D)"), &g_Config.m_TcKeystrokes, &Column, LineSize);
...
void CMenus::RenderSettingsTClientInfo(CUIRect MainView)
{
	...
	if(DoButtonLineSize_Menu(&s_GithubButton, TCLocalize("Github"), 0, &ButtonLeft, LineSize, false, 0, IGraphics::CORNER_ALL, 5.0f, 0.0f, ColorRGBA(0.0f, 0.0f, 0.0f, 0.25f)))
		s_CurCustomTab = TCLIENT_TAB_GORESBOT;
...
void CMenus::RenderSettingsGoresBot(CUIRect MainView)
{
	CUIRect Title;
	MainView.HSplitTop(HeadlineHeight, &Title, &MainView);
	Ui()->DoLabel(&Title, TCLocalize("Gores Bot"), HeadlineFontSize, TEXTALIGN_ML);
	MainView.HSplitTop(MarginSmall, nullptr, &MainView);

	// Simple overlay sections without scroll
	static std::vector<CUIRect> s_SectionBoxes;

	CUIRect LeftView, RightView, Column, Label, Button;
	MainView.VSplitMid(&LeftView, &RightView, MarginBetweenViews);
	LeftView.VSplitLeft(MarginSmall, nullptr, &LeftView);
	RightView.VSplitRight(MarginSmall, &RightView, nullptr);
	...
	s_SectionBoxes.push_back(Column);
	Column.HSplitTop(HeadlineHeight, &Label, &Column);
	Ui()->DoLabel(&Label, TCLocalize("Assist Mode"), HeadlineFontSize, TEXTALIGN_ML);
	Column.HSplitTop(MarginSmall, nullptr, &Column);

	static std::vector<const char *> s_ModeDropDownNames;
	s_ModeDropDownNames = {TCLocalize("Off"), TCLocalize("Hook Assist"), TCLocalize("Direction Assist"), TCLocalize("Both Hook & Direction")};
	...
	static int s_GoresBotMode = 0; // 0: Off, 1: Hook, 2: Direction, 3: Both

	CUIRect DropDownRect;
	Column.HSplitTop(LineSize, &DropDownRect, &Column);
	DropDownRect.VSplitLeft(80.0f, &Label, &DropDownRect);
	Ui()->DoLabel(&Label, TCLocalize("Mode:"), FontSize, TEXTALIGN_ML);
	const int ModeSelectedNew = Ui()->DoDropDown(&DropDownRect, s_GoresBotMode, s_ModeDropDownNames.data(), s_ModeDropDownNames.size(), s_ModeDropDownState);
	s_GoresBotMode = ModeSelectedNew;

	Column.HSplitTop(MarginExtraSmall, nullptr, &Column);
	s_SectionBoxes.back().h = Column.y - s_SectionBoxes.back().y;

	// ***** Tile Detection (Left Side, below Mode) ***** //
	if(s_GoresBotMode != 0)
	{
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
		Column.HSplitTop(HeadlineHeight, &Label, &Column);
		Ui()->DoLabel(&Label, TCLocalize("Tile Detection"), HeadlineFontSize, TEXTALIGN_ML);
		Column.HSplitTop(MarginSmall, nullptr, &Column);

		static int s_DetectFreeze = 0;
		DoButton_CheckBoxAutoVMarginAndSet(&s_DetectFreeze, TCLocalize("Freeze"), &s_DetectFreeze, &Column, LineSize);

		static int s_DetectDeath = 0;
		DoButton_CheckBoxAutoVMarginAndSet(&s_DetectDeath, TCLocalize("Death"), &s_DetectDeath, &Column, LineSize);

		static int s_DetectTele = 0;
		DoButton_CheckBoxAutoVMarginAndSet(&s_DetectTele, TCLocalize("Teleport"), &s_DetectTele, &Column, LineSize);

		Column.HSplitTop(MarginExtraSmall, nullptr, &Column);
		s_SectionBoxes.back().h = Column.y - s_SectionBoxes.back().y;
	}

	// Only show settings if not "Off"
	if(s_GoresBotMode == 0)
	{
		// When Off, show disabled message on right side
		Column = RightView;
		Column.HSplitTop(Margin, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
		Column.HSplitTop(HeadlineHeight, &Label, &Column);
		...
		CUIRect DisabledMsg;
		Column.HSplitTop(LineSize * 2, &DisabledMsg, &Column);
		DisabledMsg.Draw(ColorRGBA(0.3f, 0.3f, 0.3f, 0.15f), IGraphics::CORNER_ALL, 5.0f);
		DisabledMsg.HMargin(MarginSmall, &DisabledMsg);
		Ui()->DoLabel(&DisabledMsg, TCLocalize("Gores Bot is disabled.\nSelect a mode to configure settings."), FontSize, TEXTALIGN_MC);
	...
	{
		// ***** Direction Assist Settings (Right Side, first) ***** //
		if(s_GoresBotMode == 2 || s_GoresBotMode == 3)
		{
			Column = RightView;
			Column.HSplitTop(Margin, nullptr, &Column);
			s_SectionBoxes.push_back(Column);
			Column.HSplitTop(HeadlineHeight, &Label, &Column);
			Ui()->DoLabel(&Label, TCLocalize("Direction Assist"), HeadlineFontSize, TEXTALIGN_ML);
			Column.HSplitTop(MarginSmall, nullptr, &Column);

			// Direction sensitivity
			Column.HSplitTop(LineSize, &Button, &Column);
			static int s_DirectionSensitivity = 60;
			Ui()->DoScrollbarOption(&s_DirectionSensitivity, &s_DirectionSensitivity, &Button, TCLocalize("Direction Sensitivity"), 1, 100, &CUi::ms_LinearScrollbarScale, 0, "%");

			// Direction prediction time
			Column.HSplitTop(LineSize, &Button, &Column);
			static int s_DirectionPredictionTime = 150;
			Ui()->DoScrollbarOption(&s_DirectionPredictionTime, &s_DirectionPredictionTime, &Button, TCLocalize("Prediction Time"), 50, 500, &CUi::ms_LinearScrollbarScale, 0, " ms");

			// Auto-movement correction strength
			Column.HSplitTop(LineSize, &Button, &Column);
			static int s_MovementCorrectionStrength = 80;
			Ui()->DoScrollbarOption(&s_MovementCorrectionStrength, &s_MovementCorrectionStrength, &Button, TCLocalize("Correction Strength"), 10, 100, &CUi::ms_LinearScrollbarScale, 0, "%");

			// Direction assist options
			static int s_DirectionAssistEnabled = 1;
			DoButton_CheckBoxAutoVMarginAndSet(&s_DirectionAssistEnabled, TCLocalize("Enable auto-movement"), &s_DirectionAssistEnabled, &Column, LineSize);
			
			static int s_DirectionVisualIndicator = 1;
			DoButton_CheckBoxAutoVMarginAndSet(&s_DirectionVisualIndicator, TCLocalize("Show direction indicator"), &s_DirectionVisualIndicator, &Column, LineSize);

			static int s_DirectionSmoothing = 1;
			DoButton_CheckBoxAutoVMarginAndSet(&s_DirectionSmoothing, TCLocalize("Smooth direction changes"), &s_DirectionSmoothing, &Column, LineSize);

			static int s_DirectionOnlyInAir = 0;
			DoButton_CheckBoxAutoVMarginAndSet(&s_DirectionOnlyInAir, TCLocalize("Only assist when in air"), &s_DirectionOnlyInAir, &Column, LineSize);

			Column.HSplitTop(MarginExtraSmall, nullptr, &Column);
			s_SectionBoxes.back().h = Column.y - s_SectionBoxes.back().y;
		}

		// ***** Hook Assist Settings (Right Side, below Direction) ***** //
		if(s_GoresBotMode == 1 || s_GoresBotMode == 3)
		{
			if(s_GoresBotMode == 3)
			{
				// When both modes, continue from where Direction section ended
				Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
			}
			else
			{
				// When only Hook mode, start fresh
				Column = RightView;
				Column.HSplitTop(Margin, nullptr, &Column);
			}
			...
			s_SectionBoxes.push_back(Column);
			Column.HSplitTop(HeadlineHeight, &Label, &Column);
			Ui()->DoLabel(&Label, TCLocalize("Hook Assist"), HeadlineFontSize, TEXTALIGN_ML);
			Column.HSplitTop(MarginSmall, nullptr, &Column);

			// Hook sensitivity
			Column.HSplitTop(LineSize, &Button, &Column);
			static int s_HookSensitivity = 75;
			Ui()->DoScrollbarOption(&s_HookSensitivity, &s_HookSensitivity, &Button, TCLocalize("Hook Sensitivity"), 1, 100, &CUi::ms_LinearScrollbarScale, 0, "%");

			// Hook prediction distance
			Column.HSplitTop(LineSize, &Button, &Column);
			static int s_HookPredictionDistance = 300;
			Ui()->DoScrollbarOption(&s_HookPredictionDistance, &s_HookPredictionDistance, &Button, TCLocalize("Prediction Distance"), 100, 800, &CUi::ms_LinearScrollbarScale, 0, " units");

			// Hook assist angle tolerance
			Column.HSplitTop(LineSize, &Button, &Column);
			static int s_HookAngleTolerance = 15;
			Ui()->DoScrollbarOption(&s_HookAngleTolerance, &s_HookAngleTolerance, &Button, TCLocalize("Angle Tolerance"), 5, 45, &CUi::ms_LinearScrollbarScale, 0, "°");

			// Hook assist options
			static int s_HookAssistEnabled = 1;
			DoButton_CheckBoxAutoVMarginAndSet(&s_HookAssistEnabled, TCLocalize("Enable auto-hook release"), &s_HookAssistEnabled, &Column, LineSize);
			
			static int s_HookVisualIndicator = 1;
			DoButton_CheckBoxAutoVMarginAndSet(&s_HookVisualIndicator, TCLocalize("Show hook assist indicator"), &s_HookVisualIndicator, &Column, LineSize);

			static int s_HookOnlyInFreeze = 0;
			DoButton_CheckBoxAutoVMarginAndSet(&s_HookOnlyInFreeze, TCLocalize("Only assist when frozen"), &s_HookOnlyInFreeze, &Column, LineSize);
