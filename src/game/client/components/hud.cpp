...
void CHud::RenderKeystrokes()
{
	if(!g_Config.m_TcKeystrokes || !GameClient()->m_Snap.m_pLocalCharacter || !GameClient()->m_Snap.m_pLocalInfo)
		return;

	const float KeySize = 16.0f;
	const float KeySpacing = 3.0f;
	const float FontSize = 6.0f;
	const float StartX = 5.0f;
	const float StartY = m_Height - 160.0f;

	// Get current input state
	CNetObj_PlayerInput *pInput = &GameClient()->m_Controls.m_aInputData[g_Config.m_ClDummy];
	
	// Colors
	ColorRGBA ActiveColor = ColorRGBA(1.0f, 1.0f, 1.0f, 0.8f);
	ColorRGBA InactiveColor = ColorRGBA(0.3f, 0.3f, 0.3f, 0.6f);
	ColorRGBA TextColor = ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f);

	// Key positions
	float KeyY1 = StartY;
	float KeyY2 = StartY + KeySize + KeySpacing;
	
	// RMB key (top row, centered)
	float RMBX = StartX + KeySize/2;
	Graphics()->QuadsBegin();
	bool RMBPressed = pInput->m_Hook;
	Graphics()->SetColor(RMBPressed ? ActiveColor : InactiveColor);
	IGraphics::CQuadItem RMBQuad(RMBX, KeyY1, KeySize, KeySize);
	Graphics()->QuadsDrawTL(&RMBQuad, 1);
	Graphics()->QuadsEnd();
	
	// A D keys (bottom row)
	const char *aKeys[] = {"A", "D"};
	bool aKeyStates[] = {
	...
	for(int i = 0; i < 2; i++)
	{
		float KeyX = StartX + i * (KeySize + KeySpacing);
		Graphics()->QuadsBegin();
		Graphics()->SetColor(aKeyStates[i] ? ActiveColor : InactiveColor);
		IGraphics::CQuadItem KeyQuad(KeyX, KeyY2, KeySize, KeySize);
		Graphics()->QuadsDrawTL(&KeyQuad, 1);
		Graphics()->QuadsEnd();
	}
	...
	// RMB
	float RMBTextWidth = TextRender()->TextWidth(FontSize, "RMB");
	TextRender()->Text(RMBX + KeySize/2 - RMBTextWidth/2, KeyY1 + KeySize/2 - FontSize/2, FontSize, "RMB");
	
	// A D
	for(int i = 0; i < 2; i++)
	{
		float KeyX = StartX + i * (KeySize + KeySpacing);
		float TextWidth = TextRender()->TextWidth(FontSize, aKeys[i]);
		TextRender()->Text(KeyX + KeySize/2 - TextWidth/2, KeyY2 + KeySize/2 - FontSize/2, FontSize, aKeys[i]);
	}
